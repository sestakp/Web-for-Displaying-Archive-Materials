...
Description: Module with spider for downloading urbariums from VadeMeCum.
...
def parse_and_set_locations(archive_item, response):
    localities = response.xpath("//div[contains(@class, 'lokalita')]/text()").get()

    if not localities or not auxiliary.strip_or_return_none(localities):
        return

    municipalities = [muni.strip() for muni in localities.strip().split(',')]

    archive_item['locations'] = [auxiliary.get_locations_dict(municipality=muni) for muni in municipalities]
    

class VadeMeCumUrbariumsScraper(Spider):
    name = constants.sn_vademecum_urbariums
    archive = constants.a_vademecum
    first_url = "http://vademecum.soalitomerice.cz"  # To avoid getting 503 unavailable error
    start_url = "http://vademecum.soalitomerice.cz/vademecum/searchlink?myQuery=dfd6da04963b5fb6713d5107573fd25f&modeView=LIST"
    goNext = False
