...
Description: Module with spider for downloading rectification files from MZA.
...
def parse_and_set_locations(archive_item, item_dict):
    content_split = item_dict['obsah'].split("<br><br>")
    if len(content_split) == 1:
        return

    content_split = content_split[1].split("<br>")
    if len(content_split) == 1:
        return

    archive_item['locations'] = [auxiliary.get_locations_dict(municipality=muni.strip())
                                    for muni in content_split[0].split(',')]


class MzaRectificationFiles(Spider):
    """Spider for MZA's rectification files (rektifikační akta)."""
    name = constants.sn_mza_rectification_files
    archive = constants.a_mza
    start_urls = ["http://www.mza.cz/a8web/A8Apps1/ARCViewer/data/D2-Inventar.json"]
    ...
    def parse_item(self, item_dict):
        ...
        archive_item['typeOfRecord'] = constants.t_rectification_file
