...
Description: Module with spider for downloading land books from CeskeArchivy.
...
class CeskeArchivyLandBooksScraper(Spider):
    name = constants.sn_ceskearchivy_land_books
    archive = constants.a_ceskearchivy
    start_urls = [f"https://digi.ceskearchivy.cz/pages/klicova/ajaxmaterial1.php?ver=23.03.23&edit=0&role=&uzivid=2&subtyp=&doctree=18p&id=554663&start={nr}&typ=29&typ2=-1"
                  for nr in range(0, 6650 + 1, 50)]
    ...
    async def parse(self, response, **kwargs):
        """Parse the Land Books endpoint."""
        lb_onclick = response.xpath("//tr/@onclick").getall()
        lb_ids = [re.search("(?<=id=).*(?='\))", id_str).group() for id_str in lb_onclick]
        lb_links = [f"https://digi.ceskearchivy.cz/description.php?menu=4&id={lb_id}&lmenu=3"
                    for lb_id in lb_ids]
