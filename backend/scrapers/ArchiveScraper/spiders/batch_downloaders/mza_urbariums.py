...
Description: Module with spider for downloading urbariums from MZA.
...
class MzaUrbariums(Spider):
    """Spider for MZA's urbariums (urbáře)."""
    name = constants.sn_mza_urbariums
    archive = constants.a_mza
    start_urls = ["http://www.mza.cz/a8web/A8Apps1/ARCViewer/data/URB01-Inventar.json"]
    ...
    def parse_item(self, item_dict):
        ...
        # Write name
        archive_item['name'] = item_dict['obsah'].split("<br><br>")[0]

        # Write content
        archive_item['content'] = "\n\n".join(item_dict['obsah'].split("<br><br>")[1:])

        # Write years
        archive_item['yearFrom'], archive_item['yearTo'] = auxiliary.parse_years(item_dict['rok'])

        #print("link: ", item_dict['dd'])
        # Write url
